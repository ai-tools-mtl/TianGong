...
1（认证）、2（授权）、3（项目 CRUD）、4（元信息）、5（模板解析）、7（模板管理）、8（选模板建项目）、9（章节大纲）、10（AI 对话）、11（生成草稿）、12（编辑器）、14（跨章节上下文）、16（版本快照）、17（预览）、18（导出）、22（审查）、23（稳定性）、24（Rubric 自定义）、25（知识库自定义上传）、27（管理员全局内容）、28/28a-d（LLM 自定义配置）、31（创建管理员）。
...
|---|---|---|
| id | UUID PK | |
| user_id | UUID nullable | 自定义配置时是用户；全局时 null |
