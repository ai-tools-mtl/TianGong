...
天工的终极形态是陪伴一件专利的完整生命周期，而非仅交底书工具。**MVP 只实现阶段①，但架构从一开始就为全生命周期设计**，避免后续推倒重来。
...
┌─────────────────────────────────────────────────┐
│ 阶段③ 审查答复          ← 后续迭代               │
│  · 一审 OA（审查意见）录入 / AI 辅助分析          │
│  · 答复意见撰写（AI 辅助，引用历史答复策略）       │
│  · 二审、复审…多轮；授权 / 驳回                   │
└──────────────────────┬──────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────┐
...
- **不做**：阶段② 事务所协作、阶段③ 审查答复、agent 长期偏好记忆
