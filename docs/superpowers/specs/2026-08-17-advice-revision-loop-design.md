...
> 版本 v1.2（2026-08-17）。**已实施（2026-08-17，四批次合并 main：17b940d 批1核心管线 + generate checkpointer 紧急修复 / d548518 批2定位增强 / 182fc88 批3术语表 / 本批批4新颖性闭环）。探针结论记 GOTCHAS E7。**v1.0 初稿 + v1.1 自查后，v1.2 二轮逻辑审查完成源码级核验：坐实 generate 端点存在「checkpointer + interrupt_on + 无 thread_id」组合隐患（非 revise 新引入）；修正 v1.1 内部的术语指令矛盾（术语表 vs 沿用现状）；补齐维度级建议无修订入口的缺口；跨章节兜底单点化。修订清单见附录 B/C。
