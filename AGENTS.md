...
- **NLI 矛盾覆盖已临时禁用（v1.1）**：原设计用本地 NLI 小模型自动判断新旧记忆矛盾并覆盖，但实测 Infinity 的 `/classify` 端点不支持 NLI 句子对输入（只支持单句分类），故 `app/rag/nli.py` 的 `CONTRADICTION_OVERRIDE_ENABLED=False`，`judge_relation` 直接返回 neutral，save_memory 退回纯合并不删。代码与容器（端口 7999，`cross-encoder/nli-deberta-v3-base`）保留，待选定支持句子对的方案（自建微服务 / 大模型判断）后开启。热度/淘汰不受影响。
