...
- **Agent Notes 决策记录（引导而非强制）**：非 trivial 架构取舍建议留笔记于 `docs/notes/{proposed,implemented,rejected}/YYYY-MM-DD-主题.md`，四段骨架 Problem / Decision / **Alternatives considered（强制至少一条）** / Consequences，Status 行与目录互验；CI 有极简 lint 防骨架腐化。与 plans/ 的分工：plans 记「做什么怎么做」，notes 只记「为什么这么选、放弃了什么」，一份决策只住一处。示范见 `docs/notes/implemented/2026-08-27-prefix-cache-turn-reminder.md`。
