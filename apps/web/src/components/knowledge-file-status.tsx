...
/**
 * 知识文件异步向量化状态展示（plan async-knowledge-upload）。
 *
 * 三态：
 * - pending/processing：进度条 + 阶段文案（已上传 / 向量化中）
 * - ready：不渲染（正常卡片，由调用方决定是否显示完成态）
 * - failed：红色错误文案
 *
 * 用于 KnowledgeFileCard 顶部，让用户看到文件还在向量化、何时可检索。
 */
export function KnowledgeFileStatus({ kf }: { kf: KnowledgeFile }) {
  ...
  if (kf.status === 'failed') {
    return (
      <div className="flex items-start gap-1.5 rounded-md bg-destructive/5 p-2 text-[11px] text-destructive">
        ...
        <div>
          <p className="font-medium">向量化失败</p>
          {kf.error_message && (
  ...
  const stageText =
    kf.stage === 'embedding' || kf.status === 'processing'
      ? '向量化中…'
      : kf.stage === 'done'
        ? '即将就绪'
        : '已上传，排队向量化…'
