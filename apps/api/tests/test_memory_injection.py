"""记忆注入集成测试（批次 A 迁移：记忆属逐轮易变层，注入 build_turn_reminder 快照）。"""
import uuid


def _mk_section(db_session):
    """构造带 project 的 section 测试夹具（本文件各用例共用）。"""
...
def test_build_turn_reminder_includes_memories(db_session, monkeypatch):
    """build_turn_reminder 注入用户记忆段；静态 prompt 不再含。"""
    from app.ai.context_assembler import build_system_prompt, build_turn_reminder

    _, section = _mk_section(db_session)
    ...
    assert "偏好简洁风格" in reminder
    # 静态 prompt 不含记忆段（决策 D1：逐轮易变层不进前缀锚）
    assert "关于这位用户的长期记忆" not in build_system_prompt(db_session, section)


def test_build_turn_reminder_no_memories_omits_section(db_session, monkeypatch):
    """无记忆时快照不输出记忆段（不留空标题）。"""
    ...
    reminder = build_turn_reminder(db_session, section)
    assert "关于这位用户的长期记忆" not in reminder


# ===== S3-2：记忆使用规则指令（spec 2026-07-29-prompt-content-design §4 S3-2）=====

def test_build_turn_reminder_memories_has_usage_rules(db_session, monkeypatch):
    """[S3-2] 有记忆时快照注入使用规则（自然融入/不机械复读/无关则忽略）。

    断言用使用规则的独有特征词（「机械复读」「无关」），确保测的是规则而非旧文案。
    """
    from app.ai.context_assembler import build_turn_reminder
    ...
    reminder = build_turn_reminder(db_session, section)
    assert "复读" in reminder or "机械复读" in reminder  # 不要机械复读
    assert "无关" in reminder                             # 无关记忆忽略


def test_build_turn_reminder_user_input_used_as_retrieval_query(db_session, monkeypatch):
    ...
    build_turn_reminder(db_session, section, user_input="检查下我的写作风格")
...
def test_build_turn_reminder_no_user_input_falls_back_to_section_signal(db_session, monkeypatch):
    ...
    build_turn_reminder(db_session, section, user_input=None)
