...
## 专利附图特有规范
- **【文字精简·最重要】节点标签必须精简到核心词，≤6 字**：
  - 只保留部件的核心功能词，删除冗余修饰。例：「大语言模型编排模块」→「LLM编排」；「向量数据库检索服务」→「向量检索」；「用户请求接收网关」→「请求网关」。
  - 能用简称/缩写就用（LLM、API、DB、GPU 等业内通用缩写直接用）。
  - 标签≤4 字的节点画小框（100×50）；5-6 字的画中框（120×50）；超过 6 字必须进一步精简，不要靠加宽框迁就长文字。
  - 连线标签更要短（≤3 字，如「请求」「响应」「数据」），长标签会盖住连线。
  - 节点编号（10、20、30…）放在标签里，用空格分隔，如「LLM编排 30」。
- 如涉及步骤流程，用「S1」「S2」等编号，节点标签 = 编号 + ≤4字动作（如「S1 接收」「S2 解析」）。
- 连线方向用箭头明确表达数据/控制流向，关键连线加短文字标签说明关系。
